...
extern void logSerialOutputPushSuppress();
extern void logSerialOutputPopSuppress();

namespace {
struct LogSerialSuppressScope {
  LogSerialSuppressScope() { logSerialOutputPushSuppress(); }
  ~LogSerialSuppressScope() { logSerialOutputPopSuppress(); }
};
}  // namespace
#endif

// USB CDC + heavy Serial during SPI refresh can trigger spinlock asserts on ESP32-S3.
#if defined(BOARD_ESP32_S3_EPAPER_397)
#define EINK_LOG(...) ((void)0)
#else
#define EINK_LOG(...)            \
  do {                           \
...
EInkDisplay::EInkDisplay(int8_t sclk, int8_t mosi, int8_t cs, int8_t dc, int8_t rst, int8_t busy)
  ...
  EINK_LOG("[%lu] EInkDisplay: Constructor called\n", millis());
  EINK_LOG("[%lu]   SCLK=%d, MOSI=%d, CS=%d, DC=%d, RST=%d, BUSY=%d\n", millis(), sclk, mosi, cs, dc, rst, busy);
}

void EInkDisplay::begin() {
  EINK_LOG("[%lu] EInkDisplay: begin() called\n", millis());
  ...
  EINK_LOG("[%lu]   Static frame buffer (%lu bytes)\n", millis(), bufferSize);
#else
  memset(frameBuffer1, 0xFF, bufferSize);
  EINK_LOG("[%lu]   Static frame buffers (2 x %lu bytes)\n", millis(), bufferSize);
#endif

  EINK_LOG("[%lu]   Initializing e-ink display driver...\n", millis());

  // Initialize SPI with custom pins
  SPI.begin(_sclk, -1, _mosi, _cs);
  const uint32_t spiHz = _x3Mode ? 10000000 : 20000000;  // Waveshare ESP32-S3-ePaper-3.97 max 20MHz
  spiSettings = SPISettings(spiHz, MSBFIRST, SPI_MODE0);
  EINK_LOG("[%lu]   SPI initialized at %lu Hz, Mode 0\n", millis(), spiHz);
  ...
  EINK_LOG("[%lu]   GPIO pins configured\n", millis());
  ...
  EINK_LOG("[%lu]   E-ink display driver initialized\n", millis());
}
...
void EInkDisplay::resetDisplay() {
  ...
  digitalWrite(_rst, HIGH);
  delay(20);
  EINK_LOG("[%lu]   Display reset complete\n", millis());
  if (_x3Mode) {
    delay(50);
    return;
  }
...
void EInkDisplay::waitForRefresh(const char* comment) {
  ...
  if (comment) EINK_LOG("[%lu]   Refresh done: %s (%lu ms)\n", millis(), comment, millis() - start);
}
...
void EInkDisplay::waitWhileBusy(const char* comment) {
  ...
  if (comment) {
    EINK_LOG("[%lu]   Wait complete: %s (%lu ms)\n", millis(), comment, millis() - start);
  }
}
...
void EInkDisplay::initDisplayController() {
  ...
  EINK_LOG("[%lu]   Initializing SSD1677 controller...\n", millis());
  ...
  EINK_LOG("[%lu]   Clearing RAM buffers...\n", millis());
  ...
  EINK_LOG("[%lu]   SSD1677 controller initialized\n", millis());
}
...
void EInkDisplay::drawImage(const uint8_t* imageData, const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h,
  ...
  if (!frameBuffer) {
    EINK_LOG("[%lu]   ERROR: Frame buffer not allocated!\n", millis());
    return;
  }
  ...
  EINK_LOG("[%lu]   Image drawn to frame buffer\n", millis());
}
...
void EInkDisplay::drawImageTransparent(const uint8_t* imageData, const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h,
  ...
  EINK_LOG("[%lu]   Transparent image drawn to frame buffer\n", millis());
}
...
void EInkDisplay::displayBuffer(RefreshMode mode, const bool turnOffScreen) {
#if defined(BOARD_ESP32_S3_EPAPER_397)
  const LogSerialSuppressScope logSuppress;
#endif
  if (_forceFullRefreshNext) {
    mode = FULL_REFRESH;
    _forceFullRefreshNext = false;
  } else if (!_x3Mode && !isScreenOn && !turnOffScreen) {
#if defined(BOARD_ESP32_S3_EPAPER_397)
    // Waveshare SSD1677: full update on first paint (per 3.97" panel spec ~3s full / ~0.3s partial).
    mode = FULL_REFRESH;
#else
    mode = HALF_REFRESH;
#endif
  ...
  if (_x3Mode) {
    ...
    EINK_LOG("[%lu]   X3_OEM_TRIGGER=0x12\n", millis());
    sendCommand(0x12);
    ...
    if (postConditionPasses > 0) {
      ...
      for (uint8_t i = 0; i < postConditionPasses; i++) {
        ...
        EINK_LOG("[%lu]   X3_OEM_TRIGGER=0x12(cond)\n", millis());
        sendCommand(0x12);
...
#ifdef EINK_DISPLAY_SINGLE_BUFFER_MODE
  // After fast refresh only: sync RED RAM for the next differential update.
  // Full/half paths already wrote RED before refresh.
  if (mode == FAST_REFRESH) {
    setRamArea(0, 0, displayWidth, displayHeight);
    writeRamBuffer(CMD_WRITE_RAM_RED, frameBuffer, bufferSize);
  }
...
void EInkDisplay::displayWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const bool turnOffScreen) {
  EINK_LOG("[%lu]   Displaying window at (%d,%d) size (%dx%d)\n", millis(), x, y, w, h);

  // Validate bounds
  if (x + w > displayWidth || y + h > displayHeight) {
    EINK_LOG("[%lu]   ERROR: Window bounds exceed display dimensions!\n", millis());
    return;
  }

  // Validate byte alignment
  if (x % 8 != 0 || w % 8 != 0) {
    EINK_LOG("[%lu]   ERROR: Window x and width must be byte-aligned (multiples of 8)!\n", millis());
    return;
  }
  ...
  EINK_LOG("[%lu]   Window buffer size: %lu bytes (%d x %d pixels)\n", millis(), windowBufferSize, w, h);
  ...
  EINK_LOG("[%lu]   Window display complete\n", millis());
}
...
void EInkDisplay::displayGrayBuffer(const bool turnOffScreen) {
  if (_x3Mode) {
    ...
    EINK_LOG("[%lu]   X3_GRAY_MODE=gray_tuned\n", millis());
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
  // SSD1677: FAST partial after HALF/FULL corrupts the next page; use full BW+RED write + full refresh.
  setRamArea(0, 0, displayWidth, displayHeight);
  writeRamBuffer(CMD_WRITE_RAM_BW, frameBuffer, bufferSize);
  writeRamBuffer(CMD_WRITE_RAM_RED, frameBuffer, bufferSize);
  refreshDisplay(FULL_REFRESH, turnOffScreen);
#else
  refreshDisplay(FAST_REFRESH, turnOffScreen);
#endif
  setCustomLUT(false);
...
void EInkDisplay::refreshDisplay(const RefreshMode mode, const bool turnOffScreen) {
  ...
  EINK_LOG("[%lu]   Powering on display 0x%02X (%s refresh)...\n", millis(), displayMode, refreshType);
...
void EInkDisplay::setCustomLUT(const bool enabled, const unsigned char* lutData) {
  if (enabled) {
    ...
    EINK_LOG("[%lu]   Custom LUT loaded\n", millis());
  } else {
    customLutActive = false;
    EINK_LOG("[%lu]   Custom LUT disabled\n", millis());
  }
}

void EInkDisplay::deepSleep() {
  EINK_LOG("[%lu]   Preparing display for deep sleep...\n", millis());
  ...
  EINK_LOG("[%lu]   Entering deep sleep mode...\n", millis());
...
void EInkDisplay::saveFrameBufferAsPBM(const char* filename) {
  ...
  if (!file) {
    EINK_LOG("Failed to open %s for writing\n", filename);
    return;
  }
  ...
  file.close();
  EINK_LOG("Saved framebuffer to %s\n", filename);
#else
  (void)filename;
  EINK_LOG("saveFrameBufferAsPBM is not supported on Arduino builds.\n");
#endif
}
