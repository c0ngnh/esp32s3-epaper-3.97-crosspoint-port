...
static void renderCharImpl(const GfxRenderer& renderer, GfxRenderer::RenderMode renderMode,
  ...
  if (!glyph) {
    // Skip missing glyphs silently (e.g. Vietnamese diacritics in SD font) — avoids serial spam.
    return;
  }
...
void GfxRenderer::drawPixel(const int x, const int y, const bool state) const {
  ...
  // Clip out-of-range pixels silently (hot path; logging here flooded serial).
...
void GfxRenderer::drawCenteredText(const int fontId, const int y, const char* text, const bool black,
  ...
  const std::string fitted = truncatedText(fontId, text, screenW, style);
  int x = (screenW - getTextWidth(fontId, fitted.c_str(), style)) / 2;
  if (x < 0) {
    x = 0;
  }
  drawText(fontId, x, y, fitted.c_str(), black, style);
}
...
void GfxRenderer::fillRect(const int x, const int y, const int width, const int height, const bool state) const {
  ...
  int w = width;
  int h = height;
  if (x0 >= screenW || y0 >= screenH || x0 + w <= 0 || y0 + h <= 0) {
    return;
  }
  if (x0 < 0) {
    w += x0;
  ...
  for (int fillY = y0; fillY < y0 + h; fillY++) {
    drawLine(x0, fillY, x0 + w - 1, fillY, state);
  }
}
