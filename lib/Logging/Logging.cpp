...
static uint8_t logSerialSuppressDepth = 0;
#endif
...
void logPrintf(const char* level, const char* origin, const char* format, ...) {
  ...
  va_end(args);
#if defined(BOARD_ESP32_S3_EPAPER_397)
  if (logSerial && logSerialSuppressDepth == 0) {
    logSerial.print(buf);
  }
#else
  if (logSerial) {
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
void logSerialOutputPushSuppress() {
  if (logSerialSuppressDepth < 255) {
    logSerialSuppressDepth++;
  }
}

void logSerialOutputPopSuppress() {
  if (logSerialSuppressDepth > 0) {
    logSerialSuppressDepth--;
  }
}
#endif
...
bool logSerialOutputEnabled() {
#if defined(BOARD_ESP32_S3_EPAPER_397)
  return logSerialSuppressDepth == 0;
#else
  return true;
#endif
}
...
size_t MySerialImpl::write(const uint8_t b) {
  if (!logSerial || !logSerialOutputEnabled()) {
    return 1;
  }
...
size_t MySerialImpl::printf(const char* format, ...) {
  ...
  return logSerial.write(reinterpret_cast<const uint8_t*>(buf), static_cast<size_t>(len));
}
