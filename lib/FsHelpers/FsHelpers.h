...
/** Safe upload basename: no path separators, traversal, control chars, or leading '.'. */
bool isSafeUploadBasename(std::string_view name);

/** True if a path component must not be written via the web upload API. */
bool isProtectedPathComponent(std::string_view component);

/**
 * Build a normalized SD path for web upload: parentDir + filename.
 * Returns false if basename or any path component is rejected.
 * outPath is prefixed with '/' when non-empty.
 */
