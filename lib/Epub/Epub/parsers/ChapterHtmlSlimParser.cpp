...
bool ChapterHtmlSlimParser::parseAndBuildPages() {
  ...
  if (indexingProgressFn && fileSize >= MIN_SIZE_FOR_POPUP) {
    indexingProgressFn(-1);
  }
  ...
  do {
    ...
    if (indexingProgressFn && fileSize >= MIN_SIZE_FOR_POPUP && fileSize > 0) {
      const int progress = static_cast<int>((bytesRead * 100ULL) / fileSize);
      if (progress != lastReportedProgress && (progress % 5 == 0 || progress >= 99)) {
        indexingProgressFn(progress);
  ...
  if (indexingProgressFn && fileSize >= MIN_SIZE_FOR_POPUP && lastReportedProgress < 100) {
    indexingProgressFn(100);
  }
