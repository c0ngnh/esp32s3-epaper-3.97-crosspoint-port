...
int pngDrawCallback(PNGDRAW* pDraw) {
  ...
  int dstY = static_cast<int>(srcY * ctx->scaleY);
  ...
  renderDstColumnRange(*ctx->config, outXBase, dstWidth, ctx->screenWidth, dstXStart, dstXEnd);
  ...
  for (int dstX = dstXStart; dstX < dstXEnd; ++dstX) {
    ...
    if (caching) {
      cw.writePixel(outX, ditheredGray);
    }
...
bool PngToFramebufferConverter::decodeToFramebuffer(const std::string& imagePath, GfxRenderer& renderer,
  ...
  if (config.useExactDimensions && config.maxWidth > 0 && config.maxHeight > 0) {
    ...
    ctx.scaleX = static_cast<float>(ctx.dstWidth) / static_cast<float>(ctx.srcWidth);
    ctx.scaleY = static_cast<float>(ctx.dstHeight) / static_cast<float>(ctx.srcHeight);
  } else {
    // Calculate scale factor to fit within maxWidth/maxHeight
    float fitScaleX = static_cast<float>(config.maxWidth) / static_cast<float>(ctx.srcWidth);
    float fitScaleY = static_cast<float>(config.maxHeight) / static_cast<float>(ctx.srcHeight);
    const float fitScale = std::min(fitScaleX, fitScaleY);
    const float scale = fitScale > 1.0f ? 1.0f : fitScale;

    ctx.dstWidth = static_cast<int>(ctx.srcWidth * scale);
    ctx.dstHeight = static_cast<int>(ctx.srcHeight * scale);
    ctx.scaleX = scale;
    ctx.scaleY = scale;
  }
  ctx.lastDstY = -1;  // Reset row tracking

  LOG_DBG("PNG", "PNG %dx%d -> %dx%d (scale %.2f x %.2f), bpp: %d", ctx.srcWidth, ctx.srcHeight, ctx.dstWidth,
          ctx.dstHeight, ctx.scaleX, ctx.scaleY, png->getBpp());
