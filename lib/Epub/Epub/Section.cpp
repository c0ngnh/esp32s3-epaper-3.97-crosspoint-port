...
bool Section::createSectionFile(const int fontId, const float lineCompression, const bool extraParagraphSpacing,
                                ...
                                const uint16_t viewportHeight, const bool hyphenationEnabled, const bool embeddedStyle,
                                const uint8_t imageRendering, const bool focusReadingEnabled,
                                const std::function<void(int)>& indexingProgressFn) {
  ...
  ChapterHtmlSlimParser visitor(
      ...
      embeddedStyle, contentBase, imageBasePath, imageRendering, indexingProgressFn, cssParser);
...
void Section::freePageCache() {
  cachedPage_.reset();
  cachedPageIndex_ = -1;
  prefetchPage_.reset();
  prefetchPageIndex_ = -1;
}

void Section::invalidatePageCache() { freePageCache(); }

std::unique_ptr<Page> Section::loadPageUncached() {
  if (!Storage.openFileForRead("SCT", filePath, file)) {
    return nullptr;
  }
  ...
  file.seek(lutOffset + sizeof(uint32_t) * static_cast<uint32_t>(currentPage));
  uint32_t pagePos = 0;
...
bool Section::ensurePageLoaded() {
  if (cachedPage_ && cachedPageIndex_ == currentPage) {
    return true;
  }
  if (prefetchPage_ && prefetchPageIndex_ == currentPage) {
    cachedPage_ = std::move(prefetchPage_);
    cachedPageIndex_ = currentPage;
    prefetchPage_.reset();
    prefetchPageIndex_ = -1;
    LOG_DBG("SCT", "Page prefetch hit %d", currentPage);
    return cachedPage_ != nullptr;
  }
  cachedPage_ = loadPageUncached();
  cachedPageIndex_ = currentPage;
  if (cachedPage_) {
    LOG_DBG("SCT", "Page loaded from SD %d", currentPage);
  }
  return cachedPage_ != nullptr;
}

Page* Section::currentPagePtr() { return cachedPage_.get(); }

void Section::prefetchPage(const int pageIndex) {
  if (pageIndex < 0 || pageIndex >= static_cast<int>(pageCount)) {
    return;
  }
  if (pageIndex == cachedPageIndex_ || pageIndex == prefetchPageIndex_) {
    return;
  }
  const int savedPage = currentPage;
  currentPage = pageIndex;
  prefetchPage_ = loadPageUncached();
  prefetchPageIndex_ = pageIndex;
  currentPage = savedPage;
  if (prefetchPage_) {
    LOG_DBG("SCT", "Prefetched page %d", pageIndex);
  }
}
...
std::unique_ptr<Page> Section::loadPageFromSectionFile() {
  ...
  if (currentPage + 1 < static_cast<int>(pageCount)) {
