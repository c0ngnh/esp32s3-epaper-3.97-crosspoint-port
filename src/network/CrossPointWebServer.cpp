...
#include "network/NetworkStackGuard.h"
...
void CrossPointWebServer::begin() {
  ...
  NetworkStackGuard::settleLwipStack(100);
  udpActive = NetworkStackGuard::isNetworkReadyForUdp(apMode) && udp.begin(LOCAL_UDP_PORT);
...
void CrossPointWebServer::handleClient() {
  ...
  // Respond to discovery broadcasts (skip while the stack is mid-transition)
  if (udpActive && NetworkStackGuard::isNetworkReadyForUdp(apMode)) {
...
void CrossPointWebServer::handleUpload(UploadState& state) const {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (!FsHelpers::isSafeUploadBasename(state.fileName.c_str())) {
      state.error = "Invalid filename";
      LOG_ERR("WEB", "[UPLOAD] Rejected unsafe filename: %s", state.fileName.c_str());
      return;
    }
    ...
    if (!FsHelpers::resolveUploadPath(state.path.c_str(), state.fileName.c_str(), resolvedPath)) {
      state.error = "Invalid upload path";
      LOG_ERR("WEB", "[UPLOAD] Rejected unsafe path: %s / %s", state.path.c_str(), state.fileName.c_str());
      return;
    }
    const String filePath = resolvedPath.c_str();
    ...
    case WStype_TEXT: {
      ...
      if (msg.startsWith("START:")) {
        ...
        if (firstColon > 0 && secondColon > 0) {
          ...
          if (!FsHelpers::isSafeUploadBasename(wsUploadFileName.c_str())) {
            LOG_ERR("WS", "START rejected: invalid filename '%s'", wsUploadFileName.c_str());
          ...
          std::string resolvedPath;
          if (!FsHelpers::resolveUploadPath(wsUploadPath.c_str(), wsUploadFileName.c_str(), resolvedPath)) {
            LOG_ERR("WS", "START rejected: invalid path '%s' / '%s'", wsUploadPath.c_str(), wsUploadFileName.c_str());
