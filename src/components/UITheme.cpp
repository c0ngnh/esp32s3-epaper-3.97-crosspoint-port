...
namespace {
CrossPointSettings::UI_THEME activeUiTheme() {
  const uint8_t raw = SETTINGS.uiTheme;
  if (raw >= CrossPointSettings::UI_THEME_COUNT) {
    return CrossPointSettings::UI_THEME::LYRA;
  }
  return static_cast<CrossPointSettings::UI_THEME>(raw);
}
}  // namespace

UITheme::UITheme() { setTheme(activeUiTheme()); }

void UITheme::reload() { setTheme(activeUiTheme()); }
...
void UITheme::setTheme(CrossPointSettings::UI_THEME type) {
  switch (type) {
    ...
    case CrossPointSettings::UI_THEME::LYRA_3_COVERS:
      ...
      break;
    default:
      LOG_DBG("UI", "Unknown UI theme %u, using Lyra", static_cast<unsigned>(type));
      currentTheme = std::make_unique<LyraTheme>();
      currentMetrics = &LyraMetrics::values;
      break;
  }
