...
struct RecentBookCardLayout {
  int bookWidth = 0;
  int bookX = 0;
  int bookY = 0;
  int bookHeight = 0;
  int bookmarkWidth = 0;
  int bookmarkHeight = 0;
  int bookmarkX = 0;
  int bookmarkY = 0;
  bool hasCoverImage = false;
};

RecentBookCardLayout computeRecentBookCardLayout(Rect rect, const std::vector<RecentBook>& recentBooks) {
  RecentBookCardLayout layout;
  layout.bookHeight = rect.height;
  layout.bookY = rect.y;
    ...
    if (Storage.openFileForRead("HOME", coverBmpPath, file)) {
      ...
      if (bitmap.parseHeaders() == BmpReaderError::Ok) {
        ...
        if (imgWidth > 0 && imgHeight > 0) {
          ...
          layout.bookWidth = static_cast<int>(layout.bookHeight * aspectRatio);
          const int maxWidth = static_cast<int>(rect.width * 0.9f);
          if (layout.bookWidth > maxWidth) {
            layout.bookWidth = maxWidth;
          }
        } else {
          layout.bookWidth = rect.width / 2;
        }
      }
    }
  }

  if (!layout.hasCoverImage) {
    layout.bookWidth = rect.width / 2;
  }

  layout.bookX = rect.x + (rect.width - layout.bookWidth) / 2;
  layout.bookmarkWidth = layout.bookWidth / 8;
  layout.bookmarkHeight = layout.bookHeight / 5;
  layout.bookmarkX = layout.bookX + layout.bookWidth - layout.bookmarkWidth - 10;
  layout.bookmarkY = layout.bookY + 5;
  return layout;
}

void drawRecentBookCoverBitmap(GfxRenderer& renderer, const RecentBookCardLayout& layout, const std::string& coverPath,
                               bool bookSelected, bool& coverRendered, bool& coverBufferStored,
  ...
  renderer.drawBitmap(bitmap, layout.bookX, layout.bookY, layout.bookWidth, layout.bookHeight);
  renderer.drawRect(layout.bookX, layout.bookY, layout.bookWidth, layout.bookHeight);
  coverBufferStored = storeCoverBuffer();
  coverRendered = coverBufferStored;
  if (bookSelected) {
    LOG_DBG("THEME", "Drawing selection");
    renderer.drawRect(layout.bookX + 1, layout.bookY + 1, layout.bookWidth - 2, layout.bookHeight - 2);
    renderer.drawRect(layout.bookX + 2, layout.bookY + 2, layout.bookWidth - 4, layout.bookHeight - 4);
  }
}

void drawRecentBookPlaceholderCard(GfxRenderer& renderer, const RecentBookCardLayout& layout, bool bookSelected,
                                   bool hasContinueReading) {
  if (bookSelected) {
    renderer.fillRect(layout.bookX, layout.bookY, layout.bookWidth, layout.bookHeight);
  } else {
    renderer.drawRect(layout.bookX, layout.bookY, layout.bookWidth, layout.bookHeight);
  }
  ...
  const int notchDepth = layout.bookmarkHeight / 3;
  const int centerX = layout.bookmarkX + layout.bookmarkWidth / 2;
  const int xPoints[5] = {layout.bookmarkX,
                          layout.bookmarkX + layout.bookmarkWidth,
                          layout.bookmarkX + layout.bookmarkWidth,
                          centerX,
                          layout.bookmarkX};
  const int yPoints[5] = {layout.bookmarkY,
                          layout.bookmarkY,
                          layout.bookmarkY + layout.bookmarkHeight,
                          layout.bookmarkY + layout.bookmarkHeight - notchDepth,
                          layout.bookmarkY + layout.bookmarkHeight};
  renderer.fillPolygon(xPoints, yPoints, 5, !bookSelected);
}

void drawRecentBookSelectionOutline(GfxRenderer& renderer, const RecentBookCardLayout& layout) {
  renderer.drawRect(layout.bookX + 1, layout.bookY + 1, layout.bookWidth - 2, layout.bookHeight - 2);
  renderer.drawRect(layout.bookX + 2, layout.bookY + 2, layout.bookWidth - 4, layout.bookHeight - 4);
}

void drawRecentBookMetadataOverlay(GfxRenderer& renderer, Rect rect, const RecentBookCardLayout& layout,
                                   const RecentBook& book, bool bookSelected, bool coverRendered) {
  auto lines = renderer.wrappedText(UI_12_FONT_ID, book.title.c_str(), layout.bookWidth - 40, 3);
  int totalTextHeight = renderer.getLineHeight(UI_12_FONT_ID) * static_cast<int>(lines.size());
  if (!book.author.empty()) {
    totalTextHeight += renderer.getLineHeight(UI_10_FONT_ID) * 3 / 2;
  }
  int titleYStart = layout.bookY + (layout.bookHeight - totalTextHeight) / 2;
  const auto truncatedAuthor = book.author.empty()
                                   ? std::string{}
                                   : renderer.truncatedText(UI_10_FONT_ID, book.author.c_str(), layout.bookWidth - 40);

  if (coverRendered) {
    constexpr int boxPadding = 8;
    int maxTextWidth = 0;
    for (const auto& line : lines) {
      maxTextWidth = std::max(maxTextWidth, renderer.getTextWidth(UI_12_FONT_ID, line.c_str()));
    }
    if (!truncatedAuthor.empty()) {
      maxTextWidth = std::max(maxTextWidth, renderer.getTextWidth(UI_10_FONT_ID, truncatedAuthor.c_str()));
    }
  ...
  const int continueY = layout.bookY + layout.bookHeight - renderer.getLineHeight(UI_10_FONT_ID) * 3 / 2;
  if (coverRendered) {
...
void BaseTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
  ...
  constexpr int epaper397ButtonPositions[] = {25, 347, 245, 350};
  const int* buttonPositions = gpio.deviceIsX3()        ? x3ButtonPositions
                             : gpio.deviceIsEpaper397() ? epaper397ButtonPositions
                                                        : x4ButtonPositions;
...
void BaseTheme::drawSideButtonHints(const GfxRenderer& renderer, const char* topBtn, const char* bottomBtn) const {
  ...
  if (gpio.deviceIsX3() || gpio.deviceIsEpaper397()) {
    // X3 / Waveshare layout: Up on left side, Down on right side, positioned higher
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
int BaseTheme::getBoard397TopBarReservedHeight() {
  return BOARD397_TOP_BAR_HEIGHT;
}

void BaseTheme::drawBoard397HeaderRow(const GfxRenderer& renderer, const int headerY, const int sidePadding,
                                      const bool showEnvironment) {
  if (showEnvironment) {
    HalBoard397::Environment env{};
    if (board397.peekEnvironmentCached(env)) {
      char envStr[28];
      snprintf(envStr, sizeof(envStr), "%.0fC %.0f%%", env.temperatureC, env.humidityPct);
      renderer.drawText(SMALL_FONT_ID, sidePadding, headerY, envStr);
    }
  }
  ...
  if (board397.readRtcForDisplay(rtc)) {
    char dateTimeStr[24];
    snprintf(dateTimeStr, sizeof(dateTimeStr), "%02u/%02u/%04u %02u:%02u", rtc.day, rtc.month, rtc.year, rtc.hour,
            rtc.minute);
    renderer.drawCenteredText(SMALL_FONT_ID, headerY, dateTimeStr);
  }
}

void BaseTheme::drawBoard397TopStatusRow(const GfxRenderer& renderer) {
  ...
  const int headerY = orientedMarginTop + SETTINGS.screenMargin + 2;
  const int sidePadding = orientedMarginLeft + metrics.statusBarHorizontalMargin;
  drawBoard397HeaderRow(renderer, headerY, sidePadding);
}
#endif
...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  const int headerRowY = rect.y + 5;

#if defined(BOARD_ESP32_S3_EPAPER_397)
  drawBoard397HeaderRow(renderer, headerRowY, rect.x + BaseMetrics::values.contentSidePadding);
#endif

  drawBatteryRight(renderer, Rect{batteryX, headerRowY, BaseMetrics::values.batteryWidth, BaseMetrics::values.batteryHeight},
                   showBatteryPercentage);

  if (title) {
#if defined(BOARD_ESP32_S3_EPAPER_397)
    const int titleY = headerRowY + BaseMetrics::values.batteryHeight + 6;
    ...
    auto truncatedTitle = renderer.truncatedText(UI_12_FONT_ID, title,
                                                 ...
                                                 EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_12_FONT_ID, headerRowY, truncatedTitle.c_str(), true, EpdFontFamily::BOLD);
#endif
  }
...
void BaseTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  const RecentBookCardLayout layout = computeRecentBookCardLayout(rect, recentBooks);

  if (hasContinueReading && !recentBooks[0].coverBmpPath.empty() && !coverRendered) {
    const std::string coverBmpPath =
        UITheme::getCoverThumbPath(recentBooks[0].coverBmpPath, BaseMetrics::values.homeCoverHeight);
    drawRecentBookCoverBitmap(renderer, layout, coverBmpPath, bookSelected, coverRendered, coverBufferStored,
                              storeCoverBuffer);
  }

  if (!bufferRestored && !coverRendered) {
    drawRecentBookPlaceholderCard(renderer, layout, bookSelected, hasContinueReading);
  }

  if (bufferRestored && bookSelected && coverRendered) {
    drawRecentBookSelectionOutline(renderer, layout);
  }

  if (hasContinueReading) {
    drawRecentBookMetadataOverlay(renderer, rect, layout, recentBooks[0], bookSelected, coverRendered);
  } else {
    const int y = layout.bookY + (layout.bookHeight - renderer.getLineHeight(UI_12_FONT_ID) -
...
void BaseTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
                               ...
                               const std::function<UIIcon(int index)>& rowIcon, const int maxVisibleRows) const {
  (void)maxVisibleRows;
  for (int i = 0; i < buttonCount; ++i) {
    ...
    const int maxLabelWidth = std::max(0, rect.width - BaseMetrics::values.contentSidePadding * 2 - 8);
    const std::string truncatedLabel =
        renderer.truncatedText(UI_10_FONT_ID, labelStr.c_str(), maxLabelWidth, EpdFontFamily::BOLD);
    const char* label = truncatedLabel.c_str();
    ...
    renderer.drawText(UI_10_FONT_ID, textX, textY, label, selectedIndex != i, EpdFontFamily::BOLD);
  }
}
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
  drawBoard397TopStatusRow(renderer);
#endif
  ...
  if (!title.empty()) {
    ...
    const int titleMarginLeft = statusBarLeftUsed + 30;
