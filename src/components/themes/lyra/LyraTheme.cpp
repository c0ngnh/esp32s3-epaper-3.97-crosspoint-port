...
#include "components/icons/image32.h"
#include "components/icons/mic.h"
#include "components/icons/alarm.h"
...
#include "components/icons/stopwatch.h"
...
namespace {
constexpr int kLyraListValueEditChevronStripPx = 26;

void drawLyraListValueEditHint(const GfxRenderer& renderer, const int centerX, const int midY) {
  constexpr int hw = 7;
  constexpr int lineW = 3;
  const int uy = midY - 8;
  renderer.drawLine(centerX - hw, uy + hw, centerX, uy, lineW, true);
  renderer.drawLine(centerX, uy, centerX + hw, uy + hw, lineW, true);
  const int dy = midY + 8;
  renderer.drawLine(centerX - hw, dy - hw, centerX, dy, lineW, true);
  renderer.drawLine(centerX, dy, centerX + hw, dy - hw, lineW, true);
}
}  // namespace
...
void LyraTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  BaseTheme::drawBoard397HeaderRow(renderer, headerRowY, rect.x + LyraMetrics::values.contentSidePadding);
#endif
  ...
  drawBatteryRight(renderer, Rect{batteryX, headerRowY, LyraMetrics::values.batteryWidth, LyraMetrics::values.batteryHeight},
...
void LyraTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    const bool valueEditHintStrip = highlightValue && (i == selectedIndex);
    if (rowValue != nullptr) {
      valueText = rowValue(i);
      const int titleW = renderer.getTextWidth(UI_10_FONT_ID, rowTitle(i).c_str());
      const int valueCap = std::clamp(contentWidth - textX - titleW - hPaddingInSelection * 3, maxListValueWidth,
                                    contentWidth - textX - 40);
      valueText = renderer.truncatedText(UI_10_FONT_ID, valueText.c_str(), valueCap);
      const int textMeasuredW =
          valueText.empty() ? 0 : renderer.getTextWidth(UI_10_FONT_ID, valueText.c_str());
      const int stripW = (valueEditHintStrip && !valueText.empty()) ? kLyraListValueEditChevronStripPx : 0;
      valueWidth = textMeasuredW + hPaddingInSelection + stripW;
      rowTextWidth -= valueWidth;
    }

    auto itemName = rowTitle(i);
    const int safeTextWidth = std::max(0, rowTextWidth);
    auto item = renderer.truncatedText(UI_10_FONT_ID, itemName.c_str(), safeTextWidth);
    ...
    // Draw value
    if (!valueText.empty()) {
      const int innerTW = renderer.getTextWidth(UI_10_FONT_ID, valueText.c_str());

      const int valueInnerMeasure = innerTW + hPaddingInSelection;
      const int csp = LyraMetrics::values.contentSidePadding;
      const int valueTextOriginX = rect.x + contentWidth - csp - innerTW - hPaddingInSelection;
      const int boxFillX = rect.x + contentWidth - csp - hPaddingInSelection - valueInnerMeasure;
      const int boxFillW = valueInnerMeasure + hPaddingInSelection;
      ...
      if (valueEditHintStrip && highlightValue) {
        const int lineH = renderer.getLineHeight(UI_10_FONT_ID);
        const int midY = valueY + lineH / 2;
        const int cx = boxFillX - kLyraListValueEditChevronStripPx / 2;
        drawLyraListValueEditHint(renderer, cx, midY);
      }

      if (i == selectedIndex && highlightValue) {
        renderer.fillRoundedRect(boxFillX, itemY, boxFillW, rowHeight, cornerRadius, Color::Black);
      }

      renderer.drawText(UI_10_FONT_ID, valueTextOriginX, valueY, valueText.c_str(),
...
void LyraTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    const bool bookSelected = (selectorIndex >= 0 && selectorIndex == 0);
...
void LyraTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  const int visibleCap = std::max(1, maxVisibleRows);
  const int tileWidth = rect.width - LyraMetrics::values.contentSidePadding * 2;
  const int rowHeight = rect.height / visibleCap;
  const int visibleSlots = std::min(buttonCount, visibleCap);
  int scrollFirst = 0;
  if (buttonCount > visibleSlots) {
    scrollFirst = selectedIndex - (visibleSlots - 1);
    if (scrollFirst < 0) {
      scrollFirst = 0;
    }
    if (scrollFirst > buttonCount - visibleSlots) {
      scrollFirst = buttonCount - visibleSlots;
    }
    const int scrollBarX = rect.x + rect.width - LyraMetrics::values.scrollBarRightOffset;
    const int scrollAreaHeight = visibleSlots * rowHeight;
    const int scrollBarHeight = std::max(12, (scrollAreaHeight * visibleSlots) / buttonCount);
    const int maxScroll = buttonCount - visibleSlots;
    const int scrollBarY =
        rect.y + (maxScroll > 0 ? ((scrollAreaHeight - scrollBarHeight) * scrollFirst) / maxScroll : 0);
    renderer.drawLine(scrollBarX, rect.y, scrollBarX, rect.y + scrollAreaHeight, true);
  ...
  for (int vis = 0; vis < visibleSlots; ++vis) {
    const int i = scrollFirst + vis;
    if (i >= buttonCount) {
      break;
    }
    const int tileY = rect.y + vis * rowHeight;
    const Rect tileRect{rect.x + LyraMetrics::values.contentSidePadding, tileY, tileWidth, rowHeight};
    ...
    const int maxLabelWidth = std::max(0, tileRect.x + tileRect.width - textX - 4);
    const std::string truncatedLabel =
        renderer.truncatedText(UI_12_FONT_ID, labelStr.c_str(), maxLabelWidth, EpdFontFamily::BOLD);
    renderer.drawText(UI_12_FONT_ID, textX, textY, truncatedLabel.c_str(), true, EpdFontFamily::BOLD);
  }
}
