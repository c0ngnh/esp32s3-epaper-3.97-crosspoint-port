...
void KeyboardEntryActivity::loop() {
  ...
  const bool ep397 = gpio.deviceIsEpaper397();

  const auto moveCol = [this](const int delta) {
    int maxCol = isBottomRow(selectedRow) ? BOTTOM_KEY_COUNT - 1 : getContentColCount() - 1;
    const int count = maxCol + 1;
    if (delta < 0) {
      selectedCol = ButtonNavigator::previousIndex(selectedCol, count);
    } else {
      selectedCol = ButtonNavigator::nextIndex(selectedCol, count);
    }
    requestUpdate();
  };

  const auto moveRow = [this, totalRows](const int delta) {
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
  if (ep397 && halTiltSensor.isAvailable() && !cursorMode) {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Up)) {
    if (ep397 && cursorMode && upHeld && !upLongHandled) {
      if (togglePos) {
      ...
      requestUpdate();
    } else if (upHeld && !upLongHandled && !cursorMode && !ep397) {
      moveRow(-1);
    }
    upHeld = false;
    upLongHandled = false;
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::Down)) {
    ...
    if (cursorMode && !ep397) {
      ...
      requestUpdate();
    } else if (ep397 && cursorMode && inputType == InputType::Password && !togglePos) {
      rightHeld = true;
      rightLongHandled = false;
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Down)) {
    if (ep397 && cursorMode && downHeld && !downLongHandled) {
      if (!togglePos && cursorPos < text.length()) {
        cursorPos++;
        requestUpdate();
      }
    } else if (downHeld && !downLongHandled && !cursorMode && !ep397) {
      moveRow(1);
    }
    downHeld = false;
    downLongHandled = false;
  ...
  // 397: column navigation is tilt left/right only; Up/Down buttons are not used for grid nav.
  if (!ep397) {
  ...
  if (ep397 && rightHeld && !rightLongHandled && mappedInput.isPressed(MappedInputManager::Button::Down) &&
      mappedInput.getHeldTime() > LONG_PRESS_MS) {
    if (cursorMode && inputType == InputType::Password && !togglePos) {
      savedCursorPos = rightStartCursorPos;
      togglePos = true;
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
  // 397 InputManager swallows BTN_CONFIRM press events, so detect press edge manually
  // (do NOT reset confirmLongHandled every loop iteration — that re-fires the long-press handler).
  const bool confirmIsDown = mappedInput.isPressed(MappedInputManager::Button::Confirm);
  if (confirmIsDown && !confirmHeld) {
    confirmHeld = true;
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
      gpio.isPressed(HalGPIO::BTN_CONFIRM) && gpio.getHeldTime() > LONG_PRESS_MS) {
#else
  ...
  if (mappedInput.wasConfirmClicked()) {
    if (!confirmLongHandled && !cursorMode) {
      if (handleKeyPress()) {
        requestUpdate();
      }
...
void KeyboardEntryActivity::render(RenderLock&&) {
  ...
  if (hintVisible && !text.empty()) {
    ...
    if (cursorMode) {
      ...
      if (inputType == InputType::Password && togglePos) {
        ...
        renderer.drawCenteredText(SMALL_FONT_ID, hintLineY,
                                  gpio.deviceIsEpaper397() ? tr(STR_KB_HINT_397_MOVE_CURSOR)
  ...
  if (cursorMode) {
    ...
    if (gpio.deviceIsEpaper397()) {
      tipCount += halTiltSensor.isAvailable() ? 3 : 2;
    }
  }
  ...
  if (tipCount > 0) {
    ...
    if (cursorMode) {
      ...
      if (gpio.deviceIsEpaper397()) {
        if (halTiltSensor.isAvailable()) {
          drawTip(tr(STR_KB_HINT_397_NAV_TILT), y);
          y += tipsLh;
        }
        drawTip(tr(STR_KB_HINT_397_NAV_BUTTONS), y);
        y += tipsLh;
        drawTip(tr(STR_KB_HINT_397_CENTER), y);
        y += tipsLh;
      }
