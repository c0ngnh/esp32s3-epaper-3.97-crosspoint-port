...
void TxtReaderActivity::loop() {
  // Short BACK → home (center hold full refresh is handled in main).
...
void TxtReaderActivity::initializeReader() {
  ...
  cachedOrientedMarginTop += BaseTheme::getBoard397TopBarReservedHeight();
#endif
...
void TxtReaderActivity::buildPageIndex() {
  ...
  while (offset < fileSize) {
    ...
    if (fileSize > 0) {
      const int progress =
          static_cast<int>((static_cast<uint64_t>(offset) * 100ULL) / static_cast<uint64_t>(fileSize));
      if (progress != lastReportedProgress && (progress % 5 == 0 || progress >= 99)) {
        GUI.fillPopupProgress(renderer, popupRect, progress);
        lastReportedProgress = progress;
      }
    }

    // Yield to other tasks periodically (allows Back long-press to abort)
    if (pageOffsets.size() % 10 == 0) {
      vTaskDelay(1);
    }
  }
