...
void CrossPointWebServerActivity::onEnter() {
  ...
  startActivityForResult(std::make_unique<NetworkModeSelectionActivity>(renderer, mappedInput),
                         [this](const ActivityResult& result) {
                           if (result.isCancelled || !std::holds_alternative<NetworkModeResult>(result.data)) {
...
void CrossPointWebServerActivity::onNetworkModeSelected(const NetworkMode mode) {
  ...
  if (mode == NetworkMode::CONNECT_CALIBRE) {
    startActivityForResult(std::make_unique<CalibreConnectActivity>(renderer, mappedInput),
                           [this](const ActivityResult&) { activityManager.goHome(); });
    return;
  }
    ...
    startActivityForResult(std::make_unique<WifiSelectionActivity>(renderer, mappedInput),
                           [this](const ActivityResult& result) {
                             const bool connected =
                                 !result.isCancelled && std::holds_alternative<WifiResult>(result.data);
                             if (connected) {
...
void CrossPointWebServerActivity::onWifiSelectionComplete(const bool connected) {
  ...
  if (connected) {
    ...
    if (!NetworkStackGuard::waitForStaIp()) {
      LOG_ERR("WEBACT", "STA connected but no IP assigned");
      activityManager.goHome();
      return;
    }
    NetworkStackGuard::settleLwipStack(200);

    if (NetworkStackGuard::beginMdns(AP_HOSTNAME)) {
