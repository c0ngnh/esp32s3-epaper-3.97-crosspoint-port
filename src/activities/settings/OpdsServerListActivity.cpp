...
void OpdsServerListActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, screen.header, tr(STR_OPDS_SERVERS));

  if (itemCount == 0) {
    AppScreenLayout::drawBodyMessage(renderer, screen.body, tr(STR_NO_SERVERS));
  } else {
    ...
    GUI.drawList(
        renderer, screen.body, itemCount, selectedIndex,
