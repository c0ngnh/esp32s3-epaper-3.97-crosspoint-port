...
bool FontDownloadActivity::fetchAndParseManifest() {
  ...
  if (result != HttpDownloader::OK) {
    ...
    errorMessage_ = tr(STR_FONT_FETCH_FAILED);
  ...
  if (!Storage.openFileForRead("FONT", MANIFEST_TMP, manifestFile)) {
    ...
    Storage.remove(MANIFEST_TMP);
    errorMessage_ = tr(STR_FONT_READ_FAILED);
    return false;
  }
  ...
  if (version != FONTS_MANIFEST_VERSION) {
    ...
    errorMessage_ = tr(STR_FONT_MANIFEST_VERSION);
    return false;
  }
...
void FontDownloadActivity::downloadFamily(ManifestFamily& family) {
  ...
  if (!fontInstaller_.ensureFamilyDir(family.name.c_str())) {
    RenderLock lock(*this);
    state_ = ERROR;
    errorMessage_ = tr(STR_FONT_DIR_FAILED);
    return;
  }
  ...
  for (size_t i = 0; i < family.files.size(); i++) {
    ...
    if (result != HttpDownloader::OK) {
      ...
      {
        char buf[96];
        snprintf(buf, sizeof(buf), tr(STR_FONT_DOWNLOAD_FAILED), file.name.c_str());
        errorMessage_ = buf;
      }
      return;
    }
    ...
    if (!computeFileCrc32(destPath, actualCrc)) {
      ...
      {
        char buf[96];
        snprintf(buf, sizeof(buf), tr(STR_FONT_CHECKSUM_COMPUTE), file.name.c_str());
        errorMessage_ = buf;
      }
      return;
    }
    if (actualCrc != file.crc32) {
      ...
      {
        char buf[96];
        snprintf(buf, sizeof(buf), tr(STR_FONT_CHECKSUM_MISMATCH), file.name.c_str());
        errorMessage_ = buf;
      }
      return;
    }
    ...
    if (!fontInstaller_.validateCpfontFile(destPath)) {
      ...
      {
        char buf[96];
        snprintf(buf, sizeof(buf), tr(STR_FONT_FILE_INVALID), file.name.c_str());
        errorMessage_ = buf;
      }
      return;
    }
