...
bool JsonSettingsIO::saveSettings(const CrossPointSettings& s, const char* path) {
  ...
  doc["autoTimeSync"] = s.autoTimeSync;
  doc["timezoneOffsetMinutes"] = s.timezoneOffsetMinutes;
  doc["dstEnabled"] = s.dstEnabled;
  doc["outputVolume"] = s.outputVolume;
#endif

  String json;
  serializeJson(doc, json);
  if (strcmp(path, "/.crosspoint/settings.json") == 0 && Storage.exists(path)) {
    Storage.rename(path, "/.crosspoint/settings.json.bak");
  }
  return Storage.writeFile(path, json);
}
...
#if defined(BOARD_ESP32_S3_EPAPER_397)
  s.autoTimeSync = doc["autoTimeSync"] | static_cast<uint8_t>(1);
  if (s.autoTimeSync > 1) {
    s.autoTimeSync = 1;
  }
  s.timezoneOffsetMinutes = doc["timezoneOffsetMinutes"] | static_cast<int16_t>(0);
  if (s.timezoneOffsetMinutes < -720) {
    s.timezoneOffsetMinutes = -720;
  } else if (s.timezoneOffsetMinutes > 840) {
    s.timezoneOffsetMinutes = 840;
  }
  s.dstEnabled = doc["dstEnabled"] | static_cast<uint8_t>(0);
  if (s.dstEnabled > 1) {
    s.dstEnabled = 0;
  }
  s.outputVolume = doc["outputVolume"] | static_cast<uint8_t>(70);
  if (s.outputVolume > 100) {
    s.outputVolume = 100;
  }
  s.applyTimezoneToSystem();
  s.applyOutputVolume();
#endif
